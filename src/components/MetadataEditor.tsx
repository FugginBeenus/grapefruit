...
export default function MetadataEditor({ metadata, albumArt, onSave, onClose, onArtChange }: MetadataEditorProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: "rgba(0,0,0,.55)", backdropFilter: "blur(2px)" }} onClick={onClose}>
      <div className="rounded-2xl border border-line bg-panel w-[520px] max-h-[85vh] overflow-y-auto gf-in" style={{ boxShadow: "var(--shadow)" }} onClick={e => e.stopPropagation()}>
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-line">
          <h2 className="text-sm font-bold text-ink">Edit Track Info</h2>
        ...
        <div className="px-6 py-5 space-y-5">
          ...
          <div className="flex items-start gap-4">
            <div className="w-24 h-24 rounded-lg panel2 border border-line flex items-center justify-center overflow-hidden shrink-0">
              {albumArt?.has_artwork && albumArt.data ? (
                ...
                <svg className="w-8 h-8 text-ink3" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M9 9l10.5-3m0 6.553v3.75a2.25 2.25 0 01-1.632 2.163l-1.32.377a1.803 1.803 0 11-.99-3.467l2.31-.66a2.25 2.25 0 001.632-2.163zm0 0V2.25L9 5.25v10.303m0 0v3.75a2.25 2.25 0 01-1.632 2.163l-1.32.377a1.803 1.803 0 01-.99-3.467l2.31-.66A2.25 2.25 0 009 15.553z" />
                </svg>
              )}
            </div>
            <div className="flex flex-col gap-2">
              <button onClick={handleArtUpload} className="btn btn-secondary text-[12px] py-1 px-3">
                {albumArt?.has_artwork ? "Replace Art" : "Add Art"}
              </button>
              {albumArt?.has_artwork && (
                <button onClick={() => onArtChange(null)} className="btn btn-ghost text-[12px] py-1 px-3" style={{ color: "var(--err)" }}>Remove Art</button>
              )}
            </div>
          </div>
          ...
          {/* Path */}
          <div>
            <p className="font-mono text-[9px] text-ink3 uppercase tracking-[.1em] mb-1">File</p>
            <p className="text-[11px] text-ink2 break-all">{metadata.path}</p>
          </div>
        </div>
...
function Field({ label, value, onChange, type = "text", span }: {
  ...
  return (
    <div className={span === 2 ? "col-span-2" : ""}>
      <label className="block font-mono text-[9px] text-ink3 uppercase tracking-[.1em] mb-1">{label}</label>
      <input
        type={type}
        value={value}
